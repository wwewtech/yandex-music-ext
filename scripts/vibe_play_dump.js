// Click wave play button, wait, dump queue/track row structure
...
setTimeout(() => { console.log('DUMP TIMEOUT'); process.exit(1); }, 90000);
(async () => {
    ...
    // click first wave card play button
    await ws.send('Runtime.evaluate', { expression: `(() => { const b = document.querySelector('[class*="PlayButtonWithCover_playButton"]'); if (b) { b.click(); return 'clicked'; } return 'no-btn'; })()`, returnByValue: true });
    await sleep(12000);

    const expr = `(() => {
      const out = { hrefs: [], rows: [] };
      document.querySelectorAll('a[href]').forEach(a => {
        const h = a.getAttribute('href');
        if (h && /track|album/.test(h)) out.hrefs.push(h.slice(0, 70));
      });
      out.hrefs = Array.from(new Set(out.hrefs)).slice(0, 30);
      // candidate row elements
      const seen = new Set();
      document.querySelectorAll('[class*="Track"], [class*="Queue"], [class*="Sequence"], [class*="Entity"]').forEach(el => {
        const c = String(el.className);
        if (c.length > 130 || seen.has(c)) return;
        seen.add(c);
        if (el.querySelector('a[href*="track"]') || el.textContent.length < 200) {
          out.rows.push({ cls: c.slice(0, 120), tag: el.tagName, text: (el.textContent || '').trim().slice(0, 60), html: el.outerHTML.slice(0, 700) });
        }
      });
      out.rows = out.rows.slice(0, 12);
